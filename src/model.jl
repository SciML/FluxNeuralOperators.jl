...
function FourierNeuralOperator(;
    ...
    return Chain(
        Dense(ch[1], ch[2]),
        OperatorKernel(ch[2]=>ch[3], modes, σ),
        OperatorKernel(ch[3]=>ch[4], modes, σ),
        OperatorKernel(ch[4]=>ch[5], modes, σ),
        OperatorKernel(ch[5]=>ch[6], modes),
        Dense(ch[6], ch[7], σ),
        Dense(ch[7], ch[8]),
        flatten
    )
...
function MarkovNeuralOperator(;
    ...
    return Chain(
        ...
        OperatorKernel(ch[3]=>ch[4], modes, σ),
        OperatorKernel(ch[4]=>ch[5], modes, σ),
        OperatorKernel(ch[5]=>ch[6], modes, σ),
        Dense(ch[6], ch[7])
    )
end
