...
function OperatorConv(rng::AbstractRNG, ch::Pair{<:Integer, <:Integer},
    ...
    wt = scale * init_weight(rng, T, prod(_modes), in_chs, out_chs)
    tr = TR(_modes)
    ...
    if !P
        ...
        # FIXME (upstream): fft rrule doesn't work with wrapper arrays
        return @compact(; modes = _modes, weights = wt, transform = tr,
            name) do x::AbstractArray{<:Real, M} where {M}
            N_ = ndims(transform)
            # xᵀ = transmute(x, Val((ntuple(i -> i + 1, N_)..., 1, N_ + 2)))
            xᵀ = permutedims(x, (ntuple(i -> i + 1, N_)..., 1, N_ + 2))
            yᵀ = __operator_conv(xᵀ, transform, weights)
            # y = transmute(yᵀ, Val((N_ + 1, 1:N_..., N_ + 2)))
...
@inline function __apply_pattern(x_tr, weights)
    ...
    @reduce x_weighted[m, o, b]:=sum(i) x_flat[m, i, b]*weights[m, i, o]
