...
function get_dataloader(; ts::AbstractRange = LinRange(100, 11000, 10000),
    ...
    data_train, data_validate = splitobs(shuffleobs((𝐱 = data[:, :, :, 1:(end - 1)],
                                                     𝐲 = data[:, :, :, 2:end])), at = ratio)

    data = gen_data(ts, resolution = 2)
    _, data_super_res = splitobs(shuffleobs((𝐱 = data[:, :, :, 1:(end - 1)],
                                             𝐲 = data[:, :, :, 2:end])), at = ratio)
    ...
    return (training = loader_train, validation = loader_validate,
            super_res = loader_super_res)
end
...
function fit!(learner, nepochs::Int)
    fit!(learner, nepochs,
         (learner.data.training, learner.data.validation, learner.data.super_res))
end
